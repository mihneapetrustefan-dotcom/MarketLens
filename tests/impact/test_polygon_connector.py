...
class TestNormalizeTickerForPolygon(unittest.TestCase):
    def test_stocks_are_unchanged(self):
        from src.impact.polygon_connector import normalize_ticker_for_polygon
        self.assertEqual(normalize_ticker_for_polygon("NVDA", "stocks"), "NVDA")

    def test_crypto_gets_x_prefix_and_usd_suffix(self):
        from src.impact.polygon_connector import normalize_ticker_for_polygon
        self.assertEqual(normalize_ticker_for_polygon("BTC", "crypto"), "X:BTCUSD")

    def test_bvb_is_unsupported(self):
        from src.impact.polygon_connector import normalize_ticker_for_polygon
        self.assertIsNone(normalize_ticker_for_polygon("TLV", "bvb"))


class TestGetMinuteCandles(unittest.TestCase):
    def setUp(self):
        self.connector = PolygonConnector(api_key="fake-key")

    def test_parses_minute_bars_into_candles(self):
        with patch.object(self.connector, "fetch_minute_bars_raw", return_value=FAKE_RESPONSE):
            candles = self.connector.get_minute_candles(
                "NVDA", datetime(2026, 8, 20, tzinfo=timezone.utc), datetime(2026, 8, 21, tzinfo=timezone.utc))
        self.assertEqual(len(candles), 2)
        self.assertEqual(candles[0].close, 101.0)

    def test_minute_candle_uses_adjusted_close(self):
        with patch.object(self.connector, "fetch_minute_bars_raw", return_value=FAKE_RESPONSE):
            candles = self.connector.get_minute_candles(
                "NVDA", datetime(2026, 8, 20, tzinfo=timezone.utc), datetime(2026, 8, 21, tzinfo=timezone.utc))
        self.assertTrue(candles[0].uses_adjusted)

    def test_not_configured_returns_empty_without_network_call(self):
        unconfigured = PolygonConnector(api_key=None)
        result = unconfigured.get_minute_candles(
            "NVDA", datetime(2026, 8, 20, tzinfo=timezone.utc), datetime(2026, 8, 21, tzinfo=timezone.utc))
        self.assertEqual(result, [])

    def test_bad_status_returns_empty_not_raises(self):
        with patch.object(self.connector, "fetch_minute_bars_raw", return_value={"status": "ERROR"}):
            candles = self.connector.get_minute_candles(
                "NVDA", datetime(2026, 8, 20, tzinfo=timezone.utc), datetime(2026, 8, 21, tzinfo=timezone.utc))
        self.assertEqual(candles, [])

    def test_network_exception_returns_empty_not_raises(self):
        with patch.object(self.connector, "fetch_minute_bars_raw", side_effect=OSError("timeout")):
            candles = self.connector.get_minute_candles(
                "NVDA", datetime(2026, 8, 20, tzinfo=timezone.utc), datetime(2026, 8, 21, tzinfo=timezone.utc))
        self.assertEqual(candles, [])
