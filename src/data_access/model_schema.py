...
Predictions are NOT persisted by default: on this dataset they would
be one row per observation per model per window, and they are exactly
re-derivable from the stored parameters plus the feature rows. Storing
a derivable artifact that large would repeat the mistake the price
cache exists to avoid, in reverse.
